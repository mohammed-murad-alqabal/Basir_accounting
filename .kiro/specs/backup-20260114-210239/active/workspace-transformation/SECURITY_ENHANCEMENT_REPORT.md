...
final authService = <credential-fixture>(secureStorage: storage);
await authService.createAccount('testuser', 'TestPass123!');

// التحقق من وجود salt
final salt = await storage.read(key: '<credential-fixture>');
expect(salt, isNotNull);

// التحقق من قوة التشفير
final hash = await storage.read(key: '<credential-fixture>');
