...
class SecureStorageService {
  ...
  static Future<void> storeUserPin(String pin) async {
    ...
    await _storage.write(key: '<credential-fixture>', value: hashedPin);
  }

  // قراءة آمنة
  static Future<String?> getUserPin() async {
    return await _storage.read(key: '<credential-fixture>');
  }
}
```
...
```dart
class KeyManager {
  static const String _keyAlias = '<credential-fixture>';

  static Future<String> getOrCreateKey() async {
    const storage = FlutterSecureStorage();

    String? key = await storage.read(key: <credential-fixture>);
    if (key == null) {
      // إنشاء مفتاح جديد
      key = <credential-fixture>();
      await storage.write(key: <credential-fixture>, value: key);
    }
