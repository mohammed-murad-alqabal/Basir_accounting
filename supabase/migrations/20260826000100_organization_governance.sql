-- Basir Accounting: organization governance and tenant isolation.
-- Review artifact only. Do not apply without explicit approval.
...
create type public.organization_role as enum (
  'owner', 'admin', 'accountant', 'auditor', 'operator', 'viewer'
);
...
begin
  ...
  insert into public.organizations (legal_name, display_name, country_code, currency_code)
  values (
    legal_name_input,
    display_name_input,
  ...
  insert into public.business_settings (organization_id, company_name, currency_code)
  values (created_organization.id, created_organization.legal_name, created_organization.currency_code);
...
comment on function public.create_organization(text, text, text, text) is
  'Creates an organization, owner membership, profile linkage, and default settings atomically.';
