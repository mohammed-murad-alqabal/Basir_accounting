-- Basir Accounting: reference data for the first Web-enabled repository slice.
-- Local review artifact only. Depends on 20260813220000_organization_governance.sql.
-- Do not apply without a dedicated database review and explicit approval.
...
comment on table public.accounts is
  'Tenant-scoped chart of accounts. Application text IDs remain stable while organization_id enforces isolation.';
comment on table public.financial_years is
  'Tenant-scoped fiscal periods. Ledger posting and period-close immutability are introduced in a later journal migration.';
comment on table public.customers is
  'Tenant-scoped customer master data. Monetary values use numeric to avoid binary floating-point loss.';
comment on table public.vendors is
  'Tenant-scoped vendor master data. Monetary values use numeric to avoid binary floating-point loss.';
