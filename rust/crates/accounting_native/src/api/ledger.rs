...
use accounting_core::{
    JournalEntry, JournalEntryLine, EntryType, EntryStatus, TemporalJustification, StandardsJustification,
    validate_for_posting, standards::registry::StandardsRegistry,
...
pub async fn validate_journal_entry(dto: EntryDto) -> anyhow::Result<()> {
    ...
    let standards: Vec<accounting_core::standards::models::StandardEntry> = standards_repo.load_all().await?;
...
pub async fn list_journal_entries(
    ...
    let entries = ledger_repo.list_entries(limit, offset, from, to, acc_id).await?;
...
pub async fn post_journal_entry(
    ...
    let _registry: Vec<accounting_core::standards::models::StandardEntry> = standards_repo.load_all().await?;
    ...
    ledger_repo.post_entry(&entry, &audit_meta)
        .await?;
...
pub async fn reverse_journal_entry(
    ...
    let reversal_id: Uuid = ledger_repo.reverse_entry(entry_id_uuid, &reason, &audit_meta)
        .await?;
...
pub async fn log_agent_consensus(
    ...
    ledger_repo.log_agent_consensus(entry_id_uuid, &consensus_json, &audit_meta)
        .await?;
...
pub async fn get_agent_consensus(
    ...
    let entry_id_uuid = Uuid::parse_str(&entry_id)?;
    ledger_repo.get_agent_consensus(entry_id_uuid)
        .await
}
...
pub async fn list_audit_logs(
    entity_id: String,
) -> anyhow::Result<Vec<crate::api::AuditRecordDto>> {
    let pool = get_pool()?;
    let ledger_repo = PgLedgerRepository::new(pool.clone());
    ...
    Ok(records.into_iter().map(crate::api::map_audit_record_to_dto).collect())
}

// Helper to map DTO to Core Entity
fn map_dto_to_entity(dto: EntryDto) -> anyhow::Result<JournalEntry> {
    ...
    for line in dto.lines {
        ...
        core_line.original_amount = line.original_amount.and_then(|s| Decimal::from_str(&s).ok());
            
        lines.push(core_line);
    }

    let linked_entry_id = dto.linked_entry_id
    ...
    let adjustment_reason = dto.adjustment_reason.as_deref().and_then(|r| match r {
        ...
        "Reclassification" => Some(accounting_core::ledger::models::AdjustmentReason::Reclassification),
        ...
        "EstimationChange" => Some(accounting_core::ledger::models::AdjustmentReason::EstimationChange),
...
fn map_entity_to_dto(entry: accounting_core::ledger::models::JournalEntry) -> EntryDto {
    EntryDto {
        ...
        date: entry.temporal.transaction_date.format("%Y-%m-%d").to_string(),
        standard_ref: entry.standards.standard_reference,
        lines: entry.lines.into_iter().map(|l| LineDto {
