...
يظل `Decimal` هو النوع الحاكم داخل domain. تتحول القيم المالية إلى نص عشري قانوني عند حد Isar ثم تعاد عبر `Decimal.parse`، ولا تستخدم `double` كوسيط تخزين أو حساب في تدفقات الدفتر. تتطلب عملية العملة غير الأساسية معًا `originalCurrency` و`originalAmount` و`exchangeRate` وقيمة الأساس. أما عملية العملة الأساسية فيجوز أن تترك رمز العملة فارغًا، لكن إذا خزّنت مبلغًا أصليًا أو سعرًا فيلزم وجودهما معًا وبقيمة موجبة.
