...
| REQ-UX-004: سجل ميزات Atlas قابل للتتبع | `UX_REQUIREMENTS` §REQ-UX-004 | `ATLAS-FEATURE-REGISTER-001` (DRAFT؛ موافقة Product + QA معلقة) | `atlas_screen_legacy_inventory.csv` يمثل `FR-ATLAS-001…099`؛ أعيد تصنيف Atlas كمدخل legacy لا حالة تنفيذ | `atlas_register_contract_test.dart` يحرس تسلسل المجال، و4 تكرارات، و8 فجوات، ومنع COMPLETE بلا دليل | [Quality Gates 31757216700](https://github.com/mohammed-murad-alqabal/Basir_accounting/actions/runs/31757216700): `1,228` نجاح، `2` تخطٍ، `26.19%`، وتحليل `177/177` معلوماتية | PARTIAL | مراجعة Product للمعرفات المكررة/المفقودة وربط معايير قبول ذات اختبار للصفوف عالية الأثر. |
