...
| REQ-UX-001: مصدر tokens بصري واحد | `UX_REQUIREMENTS` §REQ-UX-001 | `ADR-UX-001` (DRAFT؛ موافقة المالك معلقة) | `AppColors` و`AppPalette` طبقتان حاكمتان، و`AppTheme` يحولهما إلى `ColorScheme`؛ أزيل تكرار primitives الداكنة من `AppTheme` | `app_theme_token_contract_test.dart` يثبت العقدين الفاتح والداكن | تحقق محلي: اختبار العقد ناجح؛ دليل CI للـSHA التالي معلق | PARTIAL | اعتماد ADR، وربط تحقق custom seed بالوصولية، ثم تنفيذ أي انتقال brand معتمد وفق خطة الترحيل. |
