...
class ExcelImportScreen extends ConsumerWidget {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(child: _buildPreviewTable(importState)),
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('• طبيعة الحساب تكون (Debit / مدين) أو (Credit / دائن).'),
          TextButton(
  ...
  Widget _buildPreviewTable(AsyncValue<List<ImportRow>> state) => state.when(
    data: (rows) {
      if (rows.isEmpty) {
        return const Center(child: Text('لم يتم اختيار ملف بعد أو الملف فارغ'));
      }
