...
class AuthService {
  ...
  final _authStateController = <credential-fixture><String?>.broadcast();
  ...
  Future<bool> hasAccount() async {
    try {
      final username = await secureStorage.read(key: <credential-fixture>);
  ...
  Future<BasirUser?> getCurrentUser() async {
    try {
      ...
      final roleStr = await secureStorage.read(key: '<credential-fixture>');
      final permissionsStr = await secureStorage.read(key: '<credential-fixture>');
      final warehouseId = await secureStorage.read(key: '<credential-fixture>');
      final displayName = await secureStorage.read(key: '<credential-fixture>');
  ...
  Future<void> logout() async {
    try {
      await secureStorage.write(key: <credential-fixture>, value: 'false');
      _authStateController.add(null);
    } on Exception catch (e) {
  ...
  Future<bool> isLoggedIn() async {
    try {
      final isLoggedIn = await secureStorage.read(key: <credential-fixture>);
  ...
  Future<bool> isGuest() async {
    try {
      final isGuest = await secureStorage.read(key: <credential-fixture>);
      return isGuest == 'true';
    } on Exception {
      return false;
    }
  ...
  Future<void> createAccount(
    ...
    try {
      ...
      // Save RBAC info
      await secureStorage.write(key: '<credential-fixture>', value: role.name);
      await secureStorage.write(key: '<credential-fixture>', value: warehouseId);
  ...
  Future<void> updateUserProfile({
    ...
    if (displayName != null) {
      await secureStorage.write(key: '<credential-fixture>', value: displayName);
    }
    if (role != null) {
      await secureStorage.write(key: '<credential-fixture>', value: role.name);
    }
