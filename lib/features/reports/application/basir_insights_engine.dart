...
class BasirInsightsEngine extends _$BasirInsightsEngine {
  ...
  /// Generates actionable insights based on KPIs.
  List<String> generateInsights(List<FinancialKpi> kpis) {
    ...
    for (final kpi in kpis) {
      if (kpi.name == 'Current Ratio' && kpi.value < 1.2) {
        insights.add(
          'تنبيه: نسبة السيولة منخفضة (${kpi.value}). قد تواجه صعوبة في سداد الالتزامات قصيرة الأجل.',
        );
      }

      if (kpi.name == 'Burn Rate' && kpi.trend > 0.1) {
        insights.add(
          'ملاحظة: معدل الاستنزاف النقدي زاد بنسبة ${(kpi.trend * 100).toStringAsFixed(0)}%. يفضل مراجعة المصاريف التشغيلية.',
        );
      }
