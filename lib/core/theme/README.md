...
**السلطة الحاكمة:** [`ADR-UX-001`](../../../docs/03-architecture/adrs/ADR-UX-001-canonical-design-tokens-and-runtime-overrides.md)؛ يحدد هذا الدليل الاستهلاك العملي ولا ينشئ هوية بصرية موازية.
...
- **إمكانية الوصول:** تستخدم tokens اختُبرت لها نسب تباين موثقة، مع بقاء التحقق من كل تدفق ومن لون المستخدم المخصص واجبًا مستقلًا.
- **التباين:** تظهر النسب أدناه لاقترانات معلنة؛ لا تعد هذه الوثيقة امتثالًا كاملًا لكل واجهات التطبيق أو لتجاوزات وقت التشغيل.
...
│   ├── app_colors.dart         # رموز دلالية للوضع الفاتح والمشترك
│   ├── app_palette.dart        # primitives للوضع الداكن
...
يعرّف `AppColors` الرموز الدلالية الحالية، ويعرّف `AppPalette` primitives للوضع الداكن، ويجمعهما `AppTheme` في `ColorScheme`. يفضّل المستهلك `Theme.of(context).colorScheme` داخل الواجهات، ولا ينشئ قيمة brand محلية. لا يعد `ColorCustomizationService` مصدرًا لهوية المنتج: فهو تفضيل مستخدم محلي يمرر `seedColor` في وقت التشغيل، ولا يملك بعد تحقق contrast مقبولًا لكل قيمة.[ADR-UX-001](../../../docs/03-architecture/adrs/ADR-UX-001-canonical-design-tokens-and-runtime-overrides.md)

### الألوان الأساسية (`AppColors`)
