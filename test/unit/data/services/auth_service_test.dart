...
void main() {
  ...
  group('AuthService - Login/Logout', () {
    ...
    test('should upgrade a verified legacy salted SHA-256 hash to bcrypt',
      ...
      await mockStorage.write(key: '<credential-fixture>', value: username);
      await mockStorage.write(key: '<credential-fixture>', value: legacyHash);
      await mockStorage.write(key: '${username}_salt', value: userSalt);

      final result = await authService.login(username, password);

      expect(result, isTrue);
      final upgradedHash = await mockStorage.read(key: '<credential-fixture>');
