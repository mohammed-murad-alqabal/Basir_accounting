...
void main() {
  group('AuthService Reactivity', () {
    ...
    test('initialize emits correct initial state', () async {
      // Arrange
      when(mockSecureStorage.read(key: StorageKeys.isLoggedIn))
          .thenAnswer((_) async => 'true');
      when(mockSecureStorage.read(key: StorageKeys.username))
    ...
    test('login emits username', () async {
      ...
      when(mockSecureStorage.read(key: StorageKeys.username))
          .thenAnswer((_) async => 'test_user');
      when(mockSecureStorage.read(key: StorageKeys.passwordHash))
