...
void main() {
  group('State Contrast Calculator', () {
    ...
    test('calculateRelativeLuminance for known colors', () {
      ...
      expect(
        StateContrastCalculator.calculateRelativeLuminance(const Color(0xFF808080)),
        closeTo(0.2158, 0.01),
      );
    });
    ...
    test('calculateContrastRatio for standard pairs', () {
      ...
      expect(
        StateContrastCalculator.calculateContrastRatio(Colors.black, Colors.white),
        closeTo(21.0, 0.1),
      );
      // White on Black should also be ~21:1
      expect(
        StateContrastCalculator.calculateContrastRatio(Colors.white, Colors.black),
        closeTo(21.0, 0.1),
      );
    });
    ...
    test('calculateDeltaE between same color is zero', () {
      final deltaE = StateContrastCalculator.calculateDeltaE(Colors.red, Colors.red);
    ...
    test('hasMinimumVisualDifference returns correct boolean values', () {
      ...
      expect(
        StateContrastCalculator.hasMinimumVisualDifference(Colors.red, Colors.red),
        false,
      );

      // Very different colors should pass
      expect(
        StateContrastCalculator.hasMinimumVisualDifference(Colors.white, Colors.black),
        true,
      );
    });

    // Property test: 100 random colors check contrast ratio is positive
    test('Property: Contrast ratio always positive for any color pair (100 random runs)', () {
      final random = Random();
      for (var i = 0; i < 100; i++) {
        ...
        final ratio = StateContrastCalculator.calculateContrastRatio(color1, color2);
