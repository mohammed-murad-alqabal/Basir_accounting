...
void main() {
  group('Accounting Invariants Property-Based Tests', () {
    ...
    test('JournalEntry Line Invariant: Debit/Credit Balance (100 random runs)',
        () {
      for (var i = 0; i < 100; i++) {
        ...
        // Verify Invariant
        final totalDebit = lines.fold(Decimal.zero, (sum, l) => sum + l.debit);
        final totalCredit =
        ...
        expect(
          ...
          equals(totalCredit),
          reason:
              'Run $i: Total Debit ($totalDebit) must equal Total Credit ($totalCredit)',
        );
      }
    });
    ...
    test('FX Conversion Invariant: Local = Foreign * Rate (100 random runs)',
      ...
      for (var i = 0; i < 100; i++) {
        ...
        final roundedCalculated =
            Decimal.parse(calculatedLocal.toStringAsFixed(2));

        // Tolerance check for floating point issues in intermediate steps if any
        // but Decimal should be precise.
        expect(
          line.debit,
