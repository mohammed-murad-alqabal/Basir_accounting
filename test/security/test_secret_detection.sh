...
generate_content_with_secret() {
    ...
    local secret_value="<credential-fixture>$(openssl rand -hex 16 2>/dev/null || echo 'abc123def456')"
...
@test "Example: أنماط أسرار محددة يجب اكتشافها" {
    detect_secrets "const api_key = '<credential-fixture>';"
    detect_secrets "String password = '<credential-fixture>';"
    detect_secrets "final token = '<credential-fixture>';"
    detect_secrets "const secret = '<credential-fixture>';"
    detect_secrets "Authorization: Bearer <credential-fixture>"
    detect_secrets "auth_token = '<credential-fixture>';"
}
...
@test "Property: الكشف في سياقات مختلفة" {
    ...
    # في متغير
    detect_secrets "String secret = <credential-fixture>;"
